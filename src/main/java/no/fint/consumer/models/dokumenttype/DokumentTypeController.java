...
public class DokumentTypeController {
    ...
    public DokumentTypeResources getDokumentType(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getDokumentTypeByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("DokumentType cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_ALL_DOKUMENTTYPE, client);
    ...
    @PostMapping("/$query")
    public DokumentTypeResources getDokumentTypeByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getDokumentType(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private DokumentTypeResources getDokumentTypeByOdataFilter(
        ...
        Event event = new Event(
                ...
                KodeverkActions.GET_DOKUMENTTYPE, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new DokumentTypeResources();
    
        ArrayList<DokumentTypeResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<DokumentTypeResource>>() {});
    ...
    public DokumentTypeResource getDokumentTypeBySystemId(
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_DOKUMENTTYPE, client);
        event.setOperation(Operation.READ);
