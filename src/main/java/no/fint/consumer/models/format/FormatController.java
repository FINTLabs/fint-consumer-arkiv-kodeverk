...
import no.fint.model.resource.arkiv.kodeverk.FormatResources;
...
@CrossOrigin
@RestController
@RequestMapping(name = "Format", value = RestEndpoints.FORMAT, produces = {FintRelationsMediaType.APPLICATION_HAL_JSON_VALUE, MediaType.APPLICATION_JSON_UTF8_VALUE})
public class FormatController {

    private static final String ODATA_FILTER_QUERY_OPTION = "$filter=";

    @Autowired(required = false)
    private FormatCacheService cacheService;
    ...
    public Map<String, String> getLastUpdated(@RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId) {
        if (cacheService == null) {
            throw new CacheDisabledException("Format cache is disabled.");
        }
    ...
    @GetMapping
    public FormatResources getFormat(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getFormatByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Format cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_ALL_FORMAT, client);
        ...
        Stream<FormatResource> resources;
    ...
    @PostMapping("/$query")
    public FormatResources getFormatByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getFormat(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private FormatResources getFormatByOdataFilter(
        ...
        Event event = new Event(
                ...
                KodeverkActions.GET_FORMAT, client);
        event.setOperation(Operation.READ);
        ...
        ArrayList<FormatResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<FormatResource>>() {});
    ...
    public FormatResource getFormatBySystemId(
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_FORMAT, client);
        event.setOperation(Operation.READ);
        ...
        if (cacheService != null) {
            ...
            Optional<FormatResource> format = cacheService.getFormatBySystemId(orgId, id);

            fintAuditService.audit(event, Status.CACHE_RESPONSE, Status.SENT_TO_CLIENT);

            return format.map(linker::toResource).orElseThrow(() -> new EntityNotFoundException(id));
            ...
            FormatResource format = objectMapper.convertValue(response.getData().get(0), FormatResource.class);

            fintAuditService.audit(response, Status.SENT_TO_CLIENT);

            return linker.mapAndResetLinks(format);
        }    
    }
