...
import no.fint.model.resource.arkiv.kodeverk.PartRolleResources;
...
public class PartRolleController {
    ...
    public PartRolleResources getPartRolle(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getPartRolleByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("PartRolle cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_ALL_PARTROLLE, client);
    ...
    @PostMapping("/$query")
    public PartRolleResources getPartRolleByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getPartRolle(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private PartRolleResources getPartRolleByOdataFilter(
        ...
        Event event = new Event(
                ...
                KodeverkActions.GET_PARTROLLE, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new PartRolleResources();
    
        ArrayList<PartRolleResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<PartRolleResource>>() {});
    ...
    public PartRolleResource getPartRolleBySystemId(
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_PARTROLLE, client);
        event.setOperation(Operation.READ);
        ...
        if (cacheService != null) {
            ...
            return linker.mapAndResetLinks(partrolle);
        }    
    }
