...
@Slf4j
@Api(tags = {"Saksmappetype"})
@CrossOrigin
@RestController
@RequestMapping(name = "Saksmappetype", value = RestEndpoints.SAKSMAPPETYPE, produces = {FintRelationsMediaType.APPLICATION_HAL_JSON_VALUE, MediaType.APPLICATION_JSON_UTF8_VALUE})
public class SaksmappetypeController {

    private static final String ODATA_FILTER_QUERY_OPTION = "$filter=";

    @Autowired(required = false)
    private SaksmappetypeCacheService cacheService;
    ...
    public Map<String, String> getLastUpdated(@RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId) {
        if (cacheService == null) {
            throw new CacheDisabledException("Saksmappetype cache is disabled.");
        }
    ...
    @GetMapping
    public SaksmappetypeResources getSaksmappetype(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getSaksmappetypeByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Saksmappetype cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_ALL_SAKSMAPPETYPE, client);
        ...
        Stream<SaksmappetypeResource> resources;
    ...
    @PostMapping("/$query")
    public SaksmappetypeResources getSaksmappetypeByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getSaksmappetype(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private SaksmappetypeResources getSaksmappetypeByOdataFilter(
        ...
        Event event = new Event(
                ...
                KodeverkActions.GET_SAKSMAPPETYPE, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new SaksmappetypeResources();
    
        ArrayList<SaksmappetypeResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<SaksmappetypeResource>>() {});
    ...
    public SaksmappetypeResource getSaksmappetypeBySystemId(
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_SAKSMAPPETYPE, client);
        event.setOperation(Operation.READ);
        ...
        if (cacheService != null) {
            ...
            Optional<SaksmappetypeResource> saksmappetype = cacheService.getSaksmappetypeBySystemId(orgId, id);

            fintAuditService.audit(event, Status.CACHE_RESPONSE, Status.SENT_TO_CLIENT);

            return saksmappetype.map(linker::toResource).orElseThrow(() -> new EntityNotFoundException(id));
            ...
            SaksmappetypeResource saksmappetype = objectMapper.convertValue(response.getData().get(0), SaksmappetypeResource.class);

            fintAuditService.audit(response, Status.SENT_TO_CLIENT);

            return linker.mapAndResetLinks(saksmappetype);
        }    
    }
