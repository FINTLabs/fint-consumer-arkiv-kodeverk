...
import no.fint.model.resource.arkiv.kodeverk.SaksstatusResources;
...
public class SaksstatusController {
    ...
    public SaksstatusResources getSaksstatus(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getSaksstatusByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Saksstatus cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_ALL_SAKSSTATUS, client);
    ...
    @PostMapping("/$query")
    public SaksstatusResources getSaksstatusByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getSaksstatus(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private SaksstatusResources getSaksstatusByOdataFilter(
        ...
        Event event = new Event(
                ...
                KodeverkActions.GET_SAKSSTATUS, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new SaksstatusResources();
    
        ArrayList<SaksstatusResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<SaksstatusResource>>() {});
    ...
    public SaksstatusResource getSaksstatusBySystemId(
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_SAKSSTATUS, client);
        event.setOperation(Operation.READ);
        ...
        if (cacheService != null) {
            ...
            return linker.mapAndResetLinks(saksstatus);
        }    
    }
