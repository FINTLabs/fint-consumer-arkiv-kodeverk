...
public class TilgangsrestriksjonController {
    ...
    public TilgangsrestriksjonResources getTilgangsrestriksjon(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getTilgangsrestriksjonByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Tilgangsrestriksjon cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_ALL_TILGANGSRESTRIKSJON, client);
    ...
    @PostMapping("/$query")
    public TilgangsrestriksjonResources getTilgangsrestriksjonByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getTilgangsrestriksjon(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private TilgangsrestriksjonResources getTilgangsrestriksjonByOdataFilter(
        ...
        Event event = new Event(
                ...
                KodeverkActions.GET_TILGANGSRESTRIKSJON, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new TilgangsrestriksjonResources();
    
        ArrayList<TilgangsrestriksjonResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<TilgangsrestriksjonResource>>() {});
    ...
    public TilgangsrestriksjonResource getTilgangsrestriksjonBySystemId(
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_TILGANGSRESTRIKSJON, client);
        event.setOperation(Operation.READ);
