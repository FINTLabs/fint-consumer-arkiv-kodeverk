package no.novari.fint.consumer.models.dokumentstatus;
...
public class DokumentStatusController {
    ...
    public DokumentStatusResources getDokumentStatus(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getDokumentStatusByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("DokumentStatus cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_ALL_DOKUMENTSTATUS, client);
    ...
    @PostMapping("/$query")
    public DokumentStatusResources getDokumentStatusByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getDokumentStatus(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private DokumentStatusResources getDokumentStatusByOdataFilter(
        ...
        Event event = new Event(
                ...
                KodeverkActions.GET_DOKUMENTSTATUS, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new DokumentStatusResources();
    
        ArrayList<DokumentStatusResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<DokumentStatusResource>>() {});
    ...
    public DokumentStatusResource getDokumentStatusBySystemId(
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_DOKUMENTSTATUS, client);
        event.setOperation(Operation.READ);
        ...
        if (cacheService != null) {
            ...
            return linker.mapAndResetLinks(dokumentstatus);
        }    
    }
