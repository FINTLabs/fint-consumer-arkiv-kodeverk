package no.novari.fint.consumer.models.journalposttype;
...
public class JournalpostTypeController {
    ...
    public JournalpostTypeResources getJournalpostType(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getJournalpostTypeByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("JournalpostType cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_ALL_JOURNALPOSTTYPE, client);
    ...
    @PostMapping("/$query")
    public JournalpostTypeResources getJournalpostTypeByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getJournalpostType(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private JournalpostTypeResources getJournalpostTypeByOdataFilter(
        ...
        Event event = new Event(
                ...
                KodeverkActions.GET_JOURNALPOSTTYPE, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new JournalpostTypeResources();
    
        ArrayList<JournalpostTypeResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<JournalpostTypeResource>>() {});
    ...
    public JournalpostTypeResource getJournalpostTypeBySystemId(
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_JOURNALPOSTTYPE, client);
        event.setOperation(Operation.READ);
        ...
        if (cacheService != null) {
            ...
            return linker.mapAndResetLinks(journalposttype);
        }    
    }
