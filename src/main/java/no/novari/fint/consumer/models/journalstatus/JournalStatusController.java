package no.novari.fint.consumer.models.journalstatus;
...
public class JournalStatusController {
    ...
    public JournalStatusResources getJournalStatus(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getJournalStatusByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("JournalStatus cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_ALL_JOURNALSTATUS, client);
    ...
    @PostMapping("/$query")
    public JournalStatusResources getJournalStatusByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getJournalStatus(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private JournalStatusResources getJournalStatusByOdataFilter(
        ...
        Event event = new Event(
                ...
                KodeverkActions.GET_JOURNALSTATUS, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new JournalStatusResources();
    
        ArrayList<JournalStatusResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<JournalStatusResource>>() {});
    ...
    public JournalStatusResource getJournalStatusBySystemId(
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_JOURNALSTATUS, client);
        event.setOperation(Operation.READ);
        ...
        if (cacheService != null) {
            ...
            return linker.mapAndResetLinks(journalstatus);
        }    
    }
