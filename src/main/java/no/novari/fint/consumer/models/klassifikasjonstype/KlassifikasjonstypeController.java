...
@Slf4j
@Api(tags = {"Klassifikasjonstype"})
@CrossOrigin
@RestController
@RequestMapping(name = "Klassifikasjonstype", value = RestEndpoints.KLASSIFIKASJONSTYPE, produces = {FintRelationsMediaType.APPLICATION_HAL_JSON_VALUE, MediaType.APPLICATION_JSON_UTF8_VALUE})
public class KlassifikasjonstypeController {

    private static final String ODATA_FILTER_QUERY_OPTION = "$filter=";

    @Autowired(required = false)
    private KlassifikasjonstypeCacheService cacheService;
    ...
    public Map<String, String> getLastUpdated(@RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId) {
        if (cacheService == null) {
            throw new CacheDisabledException("Klassifikasjonstype cache is disabled.");
        }
    ...
    @GetMapping
    public KlassifikasjonstypeResources getKlassifikasjonstype(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getKlassifikasjonstypeByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Klassifikasjonstype cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_ALL_KLASSIFIKASJONSTYPE, client);
        ...
        Stream<KlassifikasjonstypeResource> resources;
    ...
    @PostMapping("/$query")
    public KlassifikasjonstypeResources getKlassifikasjonstypeByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getKlassifikasjonstype(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private KlassifikasjonstypeResources getKlassifikasjonstypeByOdataFilter(
        ...
        Event event = new Event(
                ...
                KodeverkActions.GET_KLASSIFIKASJONSTYPE, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new KlassifikasjonstypeResources();
    
        ArrayList<KlassifikasjonstypeResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<KlassifikasjonstypeResource>>() {});
    ...
    public KlassifikasjonstypeResource getKlassifikasjonstypeBySystemId(
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_KLASSIFIKASJONSTYPE, client);
        event.setOperation(Operation.READ);
        ...
        if (cacheService != null) {
            ...
            Optional<KlassifikasjonstypeResource> klassifikasjonstype = cacheService.getKlassifikasjonstypeBySystemId(orgId, id);

            fintAuditService.audit(event, Status.CACHE_RESPONSE, Status.SENT_TO_CLIENT);

            return klassifikasjonstype.map(linker::toResource).orElseThrow(() -> new EntityNotFoundException(id));
            ...
            KlassifikasjonstypeResource klassifikasjonstype = objectMapper.convertValue(response.getData().get(0), KlassifikasjonstypeResource.class);
