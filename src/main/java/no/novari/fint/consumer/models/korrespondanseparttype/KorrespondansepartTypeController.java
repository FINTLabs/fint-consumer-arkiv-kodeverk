package no.novari.fint.consumer.models.korrespondanseparttype;
...
public class KorrespondansepartTypeController {
    ...
    public KorrespondansepartTypeResources getKorrespondansepartType(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getKorrespondansepartTypeByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("KorrespondansepartType cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_ALL_KORRESPONDANSEPARTTYPE, client);
    ...
    @PostMapping("/$query")
    public KorrespondansepartTypeResources getKorrespondansepartTypeByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getKorrespondansepartType(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private KorrespondansepartTypeResources getKorrespondansepartTypeByOdataFilter(
        ...
        Event event = new Event(
                ...
                KodeverkActions.GET_KORRESPONDANSEPARTTYPE, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new KorrespondansepartTypeResources();
    
        ArrayList<KorrespondansepartTypeResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<KorrespondansepartTypeResource>>() {});
    ...
    public KorrespondansepartTypeResource getKorrespondansepartTypeBySystemId(
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_KORRESPONDANSEPARTTYPE, client);
        event.setOperation(Operation.READ);
