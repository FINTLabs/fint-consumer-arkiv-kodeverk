...
public class MerknadstypeController {
    ...
    public MerknadstypeResources getMerknadstype(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getMerknadstypeByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Merknadstype cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_ALL_MERKNADSTYPE, client);
    ...
    @PostMapping("/$query")
    public MerknadstypeResources getMerknadstypeByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getMerknadstype(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private MerknadstypeResources getMerknadstypeByOdataFilter(
        ...
        Event event = new Event(
                ...
                KodeverkActions.GET_MERKNADSTYPE, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new MerknadstypeResources();
    
        ArrayList<MerknadstypeResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<MerknadstypeResource>>() {});
    ...
    public MerknadstypeResource getMerknadstypeBySystemId(
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_MERKNADSTYPE, client);
        event.setOperation(Operation.READ);
        ...
        if (cacheService != null) {
            ...
            return linker.mapAndResetLinks(merknadstype);
        }    
    }
