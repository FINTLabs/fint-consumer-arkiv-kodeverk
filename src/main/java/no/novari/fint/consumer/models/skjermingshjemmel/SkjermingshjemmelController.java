package no.novari.fint.consumer.models.skjermingshjemmel;
...
public class SkjermingshjemmelController {
    ...
    public SkjermingshjemmelResources getSkjermingshjemmel(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getSkjermingshjemmelByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Skjermingshjemmel cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_ALL_SKJERMINGSHJEMMEL, client);
    ...
    @PostMapping("/$query")
    public SkjermingshjemmelResources getSkjermingshjemmelByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getSkjermingshjemmel(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private SkjermingshjemmelResources getSkjermingshjemmelByOdataFilter(
        ...
        Event event = new Event(
                ...
                KodeverkActions.GET_SKJERMINGSHJEMMEL, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new SkjermingshjemmelResources();
    
        ArrayList<SkjermingshjemmelResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<SkjermingshjemmelResource>>() {});
    ...
    public SkjermingshjemmelResource getSkjermingshjemmelBySystemId(
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_SKJERMINGSHJEMMEL, client);
        event.setOperation(Operation.READ);
        ...
        if (cacheService != null) {
            ...
            return linker.mapAndResetLinks(skjermingshjemmel);
        }    
    }
