package no.novari.fint.consumer.models.tilgangsgruppe;
...
@Slf4j
@Api(tags = {"Tilgangsgruppe"})
@CrossOrigin
@RestController
@RequestMapping(name = "Tilgangsgruppe", value = RestEndpoints.TILGANGSGRUPPE, produces = {FintRelationsMediaType.APPLICATION_HAL_JSON_VALUE, MediaType.APPLICATION_JSON_UTF8_VALUE})
public class TilgangsgruppeController {

    private static final String ODATA_FILTER_QUERY_OPTION = "$filter=";

    @Autowired(required = false)
    private TilgangsgruppeCacheService cacheService;
    ...
    public Map<String, String> getLastUpdated(@RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId) {
        if (cacheService == null) {
            throw new CacheDisabledException("Tilgangsgruppe cache is disabled.");
        }
    ...
    @GetMapping
    public TilgangsgruppeResources getTilgangsgruppe(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getTilgangsgruppeByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Tilgangsgruppe cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_ALL_TILGANGSGRUPPE, client);
        ...
        Stream<TilgangsgruppeResource> resources;
    ...
    @PostMapping("/$query")
    public TilgangsgruppeResources getTilgangsgruppeByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getTilgangsgruppe(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private TilgangsgruppeResources getTilgangsgruppeByOdataFilter(
        ...
        Event event = new Event(
                ...
                KodeverkActions.GET_TILGANGSGRUPPE, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new TilgangsgruppeResources();
    
        ArrayList<TilgangsgruppeResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<TilgangsgruppeResource>>() {});
    ...
    public TilgangsgruppeResource getTilgangsgruppeBySystemId(
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_TILGANGSGRUPPE, client);
        event.setOperation(Operation.READ);
        ...
        if (cacheService != null) {
            ...
            Optional<TilgangsgruppeResource> tilgangsgruppe = cacheService.getTilgangsgruppeBySystemId(orgId, id);

            fintAuditService.audit(event, Status.CACHE_RESPONSE, Status.SENT_TO_CLIENT);

            return tilgangsgruppe.map(linker::toResource).orElseThrow(() -> new EntityNotFoundException(id));
            ...
            TilgangsgruppeResource tilgangsgruppe = objectMapper.convertValue(response.getData().get(0), TilgangsgruppeResource.class);

            fintAuditService.audit(response, Status.SENT_TO_CLIENT);

            return linker.mapAndResetLinks(tilgangsgruppe);
        }    
    }
