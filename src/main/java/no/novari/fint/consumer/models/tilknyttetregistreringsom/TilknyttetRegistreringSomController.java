package no.novari.fint.consumer.models.tilknyttetregistreringsom;
...
public class TilknyttetRegistreringSomController {
    ...
    public TilknyttetRegistreringSomResources getTilknyttetRegistreringSom(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getTilknyttetRegistreringSomByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("TilknyttetRegistreringSom cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_ALL_TILKNYTTETREGISTRERINGSOM, client);
    ...
    @PostMapping("/$query")
    public TilknyttetRegistreringSomResources getTilknyttetRegistreringSomByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getTilknyttetRegistreringSom(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private TilknyttetRegistreringSomResources getTilknyttetRegistreringSomByOdataFilter(
        ...
        Event event = new Event(
                ...
                KodeverkActions.GET_TILKNYTTETREGISTRERINGSOM, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new TilknyttetRegistreringSomResources();
    
        ArrayList<TilknyttetRegistreringSomResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<TilknyttetRegistreringSomResource>>() {});
    ...
    public TilknyttetRegistreringSomResource getTilknyttetRegistreringSomBySystemId(
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_TILKNYTTETREGISTRERINGSOM, client);
        event.setOperation(Operation.READ);
        ...
        if (cacheService != null) {
            ...
            return linker.mapAndResetLinks(tilknyttetregistreringsom);
        }    
    }
