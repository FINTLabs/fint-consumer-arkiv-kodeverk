...
public class VariantformatController {
    ...
    public VariantformatResources getVariantformat(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getVariantformatByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Variantformat cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_ALL_VARIANTFORMAT, client);
    ...
    @PostMapping("/$query")
    public VariantformatResources getVariantformatByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getVariantformat(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private VariantformatResources getVariantformatByOdataFilter(
        ...
        Event event = new Event(
                ...
                KodeverkActions.GET_VARIANTFORMAT, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new VariantformatResources();
    
        ArrayList<VariantformatResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<VariantformatResource>>() {});
    ...
    public VariantformatResource getVariantformatBySystemId(
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_VARIANTFORMAT, client);
        event.setOperation(Operation.READ);
